...
<div class="grid_10">
    <div class="box round first grid">
        ...
        if(!empty($title))
        {
            $queryU = "UPDATE post SET title='$title' WHERE id='$id_post'";
            $request = $db->update($queryU);
        }
        if(!empty($category_id))
        {
            $queryU = "UPDATE post SET  category_id='$category_id' WHERE id='$id_post'";
            $request = $db->update($queryU);
        }
        if(!empty($author))
        {
            $queryU = "UPDATE post SET author='$author' WHERE id='$id_post'";
            $request = $db->update($queryU);
        }
        if(!empty($tags))
        {
            $queryU = "UPDATE post SET tags='$tags' WHERE id='$id_post'";
            $request = $db->update($queryU);
        }
        if(!empty($body))
        {
            $queryU = "UPDATE post SET body='$body' WHERE id='$id_post'";
            $request = $db->update($queryU);
        }
        if(!empty($fichier))
        {
            ...
            if(!isset($erreur)) //S'il n'y a pas d'erreur, on upload
            {
                ...
                {
                    $strQue = $dossier . $fichier;
                    $queryU = "UPDATE post SET image='$strQue' WHERE id='$id_post'";
        ...
        <div class="block">
            ...
            $requete = $db->select("SELECT * FROM post WHERE id='$id_post'");
            if($requete)
            {
